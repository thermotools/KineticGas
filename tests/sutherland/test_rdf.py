...
@pytest.mark.parametrize('T', T_lst)
@pytest.mark.parametrize('p', p_lst)
def test_mie_g2(comps, i, j, T, p, silent=False):
    mie = MieKinGas(comps)
    smie = S_MieKinGas(comps)
    if (i >= mie.ncomps) or (j >= mie.ncomps):
        return
...
@pytest.mark.parametrize('T', T_lst)
@pytest.mark.parametrize('p', p_lst)
def test_mie_g(comps, i, j, T, p, silent=False):
    mie = MieKinGas(comps)
    smie = S_MieKinGas(comps)
    if (i >= mie.ncomps) or (j >= mie.ncomps):
        return
    ...
    assert equal(g_mie[i][j], g_suth[i][j], tol=1e-1)
    print('Test passed\n')
...
def test_mie_d2(comps, i, j, T, p, silent=False):
    ...
    mie_2d = np.array(mie.cpp_kingas.da2_div_chi_drho(rho, T, z))
    suth_2d = np.array(smie.cpp_kingas.da2_div_chi_drho(rho, T, z))
    # print(g1_mie)
    # print(g1_suth)
    if silent is False:
        ...
        print(f'(i, j) = ({i}, {j})')
        print(f'Mie  : {mie_2d}')
        print(f'Suth : {suth_2d}')
        print(f'Diff : {(mie_2d - suth_2d) / mie_2d}')
    if mie_2d[i][j] == 0: return
    ...
    for comps in complist:
        for T in T_lst:
            for p in p_lst:
                for i in idx_list:
                    for j in idx_list:
                        test_mie_g2(comps, i, j, T, p, silent=False)
                        # print('Test passed!\n')
